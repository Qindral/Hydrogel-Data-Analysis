...
# SEM image
SEM_IMAGE = Path(r"D:\SEM\2026_02-02_REM_20mg_42mg_hydrogel\20mg_50nm_80kx_001.tif")

# Output
SAVE_PATH = None#Path(r"E:\PhD Data Analysis\SPT 2025 II\Visualizations")
...
TRAJ_IMAGE_ALPHA = 0.3  # Opacity of trajectory image in overlay
SEM_ALPHA = 0.7  # Opacity of SEM tiles
SEM_BRIGHTNESS_BOOST = 1.5  # Boost SEM brightness after inversion (1.0 = no change)
SPT_CONTRAST_LOW_PCT = 66.0  # Lower percentile for contrast stretch
SPT_CONTRAST_HIGH_PCT = 77.0  # Upper percentile for contrast stretch
...
def load_tiff_stack(tif_path: Path) -> np.ndarray:
    """Load TIFF as (T, Y, X) stack, preserving bit depth."""
...
def load_tiff_from_result_dict(result_dict: dict) -> np.ndarray | None:
    """Load TIFF stack from result_dict's tif_path."""
    ...
    if not tif_path.exists():
        return None
    return load_tiff_stack(tif_path)
...
def select_frame_for_particle(traj_df, method: str = "last") -> int:
    """Select a frame index from the trajectory (last/middle/random)."""
    frames = traj_df["frame"].to_numpy()
    if frames.size == 0:
        return 0
...
def get_spt_frame(spt_data: np.ndarray, frame_index: int) -> tuple[np.ndarray, int]:
    """Return a 2D SPT frame and a validated frame index."""
    if spt_data.ndim == 3:
        n_frames = spt_data.shape[0]
        idx = int(np.clip(frame_index, 0, n_frames - 1))
        return spt_data[idx], idx
    if spt_data.ndim == 2:
        return spt_data, 0
...
def scale_sem_to_spt(sem_image: np.ndarray, sem_px_nm: float, spt_mpp_um: float,
    ...
    # Scale factor for SEM: x = mpp_SEM / mpp_SPT
    # (i.e., SEM pixel size divided by SPT pixel size)
    scale_factor = sem_px_nm / spt_px_nm
    ...
    scaled_w = max(1, int(round(w_src * scale_factor)))
    scaled_h = max(1, int(round(h_src * scale_factor)))
    ...
    if scaled_w > max_tile_px or scaled_h > max_tile_px:
        ...
        crop_w = min(w_src, max(1, int(max_tile_px / scale_factor)))
        crop_h = min(h_src, max(1, int(max_tile_px / scale_factor)))
...
def enhance_contrast_percentile(
    ...
    """Enhance contrast using percentile stretch + optional gamma, output uint8."""
    if image.size == 0:
        return image
...
def add_pixel_grid(
    ...
    """Overlay a fine grid that outlines SPT pixels."""
    if step_px < 1.0:
        return
    offset = 0.5 * step_px
    xs = np.arange(offset, float(width) + step_px, step_px)
    ys = np.arange(offset, float(height) + step_px, step_px)
    if xs.size + ys.size > max_lines:
        dec = int(np.ceil((xs.size + ys.size) / max_lines))
        xs = xs[::dec]
        ys = ys[::dec]
    ax.vlines(xs, 0, height, colors=color, alpha=alpha, linewidth=linewidth)
    ax.hlines(ys, 0, width, colors=color, alpha=alpha, linewidth=linewidth)
...
def _plot_trajectory_style(
    ...
    """Plot a trajectory using the style from trajectory_plotter.py."""
    n_points = len(x_traj)
    if n_points < 2:
        return

    if fading:
        denom = max(n_points - 2, 1)
        for i in range(n_points - 1):
            t = i / denom
            alpha = alpha_past + (alpha_now - alpha_past) * t
            ax.plot(
                x_traj[i:i + 2],
                y_traj[i:i + 2],
                color=color,
...
def plot_single_trajectory(
    ...
    """
    Plot a single trajectory on given axes using the trajectory_plotter style.
    """
    ...
    _plot_trajectory_style(
        ...
        linewidth=linewidth,
        alpha_now=alpha,
        alpha_past=TRAJECTORY_ALPHA_PAST,
        fading=fading,
...
def create_overlay_figure(
    ...
    # Use first frame if a stack was passed
    if traj_image.ndim == 3:
        traj_image = traj_image[0]

    # Crop trajectory image and enhance contrast for visibility
    traj_crop = traj_image[y_min:y_max, x_min:x_max]
    traj_crop = enhance_contrast_percentile(
        traj_crop,
        low_pct=SPT_CONTRAST_LOW_PCT,
        high_pct=SPT_CONTRAST_HIGH_PCT,
        gamma=SPT_CONTRAST_GAMMA
    )
...
def create_scaled_single_trajectory_overlay(
    sem_image: np.ndarray,
    sem_px_nm: float,
    spt_data: np.ndarray,
    tracks_df,
    ...
    """
    Create overlay with physically correct scaling and a single trajectory.
    The SPT crop stays at native resolution; SEM is scaled to match physical size.
    """
    # Select a random trajectory with at least 10 points
    track_lengths = tracks_df.groupby('particle').size()
    valid_tracks = track_lengths[track_lengths >= 10].index.tolist()
    if not valid_tracks:
        particle_id = track_lengths.idxmax()
    else:
        particle_id = random.choice(valid_tracks)

    traj = tracks_df[tracks_df['particle'] == particle_id].sort_values('frame')
    print(f"\nSelected trajectory: {particle_id} ({len(traj)} frames)")

    frame_index = select_frame_for_particle(traj, FRAME_SELECTION)
    spt_frame, frame_index = get_spt_frame(spt_data, frame_index)
    print(f"  Using frame: {frame_index}")

    # Get trajectory bounds with padding
    padding_um = 2.0  # µm padding around trajectory
    padding_px = padding_um / spt_mpp_um

    x_min = int(max(0, traj['x'].min() - padding_px))
    x_max = int(min(spt_frame.shape[1], traj['x'].max() + padding_px))
    y_min = int(max(0, traj['y'].min() - padding_px))
    y_max = int(min(spt_frame.shape[0], traj['y'].max() + padding_px))
    ...
    spt_crop_physical_um_x = crop_w * spt_mpp_um
    spt_crop_physical_um_y = crop_h * spt_mpp_um

    print(f"  Crop: {crop_w} x {crop_h} px")
    print(f"  SPT crop physical size: {spt_crop_physical_um_x:.2f} x {spt_crop_physical_um_y:.2f} µm")
    print(f"\n  SPT pixel: {spt_mpp_um:.4f} µm/px ({spt_px_nm:.2f} nm/px)")
    ...
    sem_green_native_disp = sem_green_native.astype(np.float32) / 255.0
    ...
    # Trajectory coordinates relative to crop
    ...
    # 2. SPT crop with trajectory
    ax2 = axes[0, 1]
    ax2.imshow(spt_crop, cmap='gray')
    if SPT_GRID_ENABLED:
        add_pixel_grid(
            ax2,
            width=crop_w,
            height=crop_h,
            step_px=1.0,
            color=SPT_GRID_COLOR,
            alpha=SPT_GRID_ALPHA,
            linewidth=SPT_GRID_LINEWIDTH,
            max_lines=SPT_GRID_MAX_LINES,
        )
    _plot_trajectory_style(
        ax2,
        x_traj_spt,
        ...
        alpha_now=TRAJECTORY_ALPHA,
        alpha_past=TRAJECTORY_ALPHA_PAST,
        fading=True,
        show_current=True,
    )
    add_scalebar(ax2, spt_mpp_um)
    ax2.set_title(f'SPT + Trajectory ({len(traj)} frames)')
    ax2.axis('off')
    ...
    ax3.set_xlim(0, crop_w)
    ax3.set_ylim(crop_h, 0)
    add_scalebar(ax3, spt_mpp_um)
    ax3.set_title('Overlay (SPT + SEM)')
    ax3.axis('off')
    ...
    ax4.set_ylim(crop_h, 0)
    _plot_trajectory_style(
        ax4,
        x_traj_spt,
        y_traj_spt,
        color=TRAJECTORY_COLOR,
        linewidth=TRAJECTORY_LINEWIDTH + 0.5,
        alpha_now=1.0,
        alpha_past=TRAJECTORY_ALPHA_PAST,
        fading=False,
        show_current=True,
    )
    add_scalebar(ax4, spt_mpp_um)
    ax4.set_title(f'Overlay + Trajectory\n(FOV: {spt_crop_physical_um_x:.1f} µm)')
    ax4.axis('off')

    plt.suptitle(f'Physically Scaled Overlay\nParticle {particle_id}', fontsize=14)
    plt.tight_layout()

    if save_path:
        save_path.mkdir(parents=True, exist_ok=True)
        fig.savefig(save_path / f"scaled_overlay_particle_{particle_id}.png", dpi=200)
        print(f"Saved: {save_path / f'scaled_overlay_particle_{particle_id}.png'}")
...
def create_simple_overlay(sem_image: np.ndarray, traj_image: np.ndarray,
    ...
    Shows all trajectories on the full SPT image.
    """
    # Use a reasonable square size
    target_size = 800

    # Get original SPT image dimensions for trajectory scaling
    h_orig, w_orig = traj_image.shape[:2]
    ...
    # 1. SEM Inverted Green
    ax1 = axes[0, 0]
    ax1.imshow(sem_green)
    ax1.set_title('SEM Inverted (Green)')
    ax1.axis('off')

    # 2. SPT Image with all trajectories
    ax2 = axes[0, 1]
    ax2.imshow(traj_resized, cmap='gray')
    ...
    for pid in tracks_df['particle'].unique():
        ...
        x_scaled = traj['x'].values * scale_x
        y_scaled = traj['y'].values * scale_y
        ax2.plot(x_scaled, y_scaled, color='red', linewidth=0.5, alpha=0.7)
    ax2.set_title(f'SPT + Trajectories ({n_tracks} tracks)')
    ax2.axis('off')

    # 3. Overlay without trajectories
    ax3 = axes[1, 0]
    ax3.imshow(blended)
    ax3.set_title('Overlay (SPT + SEM)')
    ax3.axis('off')

    # 4. Overlay with all trajectories
    ax4 = axes[1, 1]
    ax4.imshow(blended)
    # Draw all trajectories
    for pid in tracks_df['particle'].unique():
        ...
        x_scaled = traj['x'].values * scale_x
        y_scaled = traj['y'].values * scale_y
        ax4.plot(x_scaled, y_scaled, color='yellow', linewidth=0.5, alpha=0.8)
    ax4.set_title(f'Overlay + Trajectories ({n_tracks} tracks)')
    ax4.axis('off')

    plt.suptitle('SPT + SEM Overlay', fontsize=14)
    plt.tight_layout()

    if save_path:
        save_path.mkdir(parents=True, exist_ok=True)
        fig.savefig(save_path / "simple_overlay.png", dpi=200)
...
def main():
    ...
    spt_stack = load_tiff_from_result_dict(result_dict)
    if spt_stack is None:
        # Try parent directory
        tif_path = TRAJ_XML.parent.parent / (TRAJ_XML.stem.replace('_Tracks', '') + '.tif')
        if tif_path.exists():
            print(f"  TIFF (found): {tif_path.name}")
            spt_stack = load_tiff_stack(tif_path)
        else:
    ...
    # Physically scaled overlay with single trajectory
    create_scaled_single_trajectory_overlay(
        sem_image, sem_px_nm, spt_stack, tracks_df, mpp_um, SAVE_PATH
    )
